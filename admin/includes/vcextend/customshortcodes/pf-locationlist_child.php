<?php
add_shortcode( 'pf_llist_widget', 'pf_llist_widget_func_child' );

function pf_llist_widget_func_child( $atts ) {
	...
	if ( ! empty( $listing_terms ) && ! is_wp_error( $listing_terms ) ) {
	    ...
	    $term_list = '<ul class="pointfinder-terms-archive pf-row">';
          ...
          if ($term->parent == 0) {
            ...
            $term_list .= '<a href="' . get_term_link( $term ) . '?type-annonce=1134" title="' . sprintf( __( 'View all posts under %s', 'pointfindert2d' ), $term->name ) . '"'.$style_text_main_custom.''.$hover_text.'>'. $iconimage_url . $term->name . ' ';
