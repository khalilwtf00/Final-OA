...
function PFGetAgentPageCol1($author_id){
		...
		$tabinside .= '<section role="itempagesidebarinfo" class="pf-itempage-sidebarinfo pfpos2 pf-itempage-elements pf-authordetail-page">';
			...
			$tabinside .= '<div id="pf-itempage-sidebarinfo">';
				...
				$tabinside .= '<div class="pf-itempage-sidebarinfo-userdetails pfpos2">
					...
					if(!empty($user_description)){
						$tabinside .= '<li class="pf-itempage-sidebarinfo-eldesc pf-itempage-sidebarinfo-elitem">'.apply_filters('the_content', get_the_content()).'</li>';
					}
            ...
            if(strpos($user_web, "http://") === false
              ...
              $user_web = 'http://'.$user_web;
						...
						$tabinside .= '<li class="pf-itempage-sidebarinfo-elurl pf-itempage-sidebarinfo-elitem"><a href="'.$user_web.'" target="_blank" rel="nofollow"><i class="pfadmicon-glyph-592"></i> '.$user_web.'</a></li>';
					}
					$tabinside .= '</ul>
				</div>';
				$tabinside .= '</div></div>';
  ...
  echo '<div id="pfauthor-items" style="display:none;">';
  if (!empty($agentposts)) {
    ...
    echo do_shortcode('[pf_itemgrid2 posts_in="'.$agentposts.'" orderby="date" sortby="DESC" items="'.$setup42_authorpagedetails_defaultppptype.'" cols="3" grid_layout_mode="'.$setup42_authorpagedetails_grid_layout_mode.'" filters="false" itemboxbg="'.$setup42_authorpagedetails_background2.'" agentmode="1" author="'.$author_id.'"]' );
