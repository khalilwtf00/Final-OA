...
$inv_date = str_replace(array_keys($months), array_values($months), $inv_date);
      ...
      if(empty(trim($inv_user_title))){
        $inv_user_title = $user->display_name;
      }
      ...
      $inv_user_city = (isset($user_meta['user_city'][0]))?$user_meta['user_city'][0]:'';
      $inv_user_company = (isset($user_meta['company'][0]))?$user_meta['company'][0]:'';
      $inv_user_postalcode = (isset($user_meta['user_postalcode'][0]))?$user_meta['user_postalcode'][0]:'';
      ...
      $pointfinder_invoice_amount_raw = get_post_meta( $params['invoiceid'], 'pointfinder_invoice_amount', true );
      $pack_price_tag = 0;
      $package_meta = FALSE;
      ...
      if (isset($invoice_meta['pointfinder_invoice_packageid'][0])) {
        ...
        $pack_price_tag = pointfinder_reformat_pricevalue_for_frontend($package_meta['webbupointfinder_mp_price']);
        ...
        $pack_price_tag = pointfinder_reformat_pricevalue_for_frontend($pointfinder_invoice_amount_raw);
      ...
      $total_pr = $pointfinder_invoice_amount_raw;
      ...
      $pack_qst = 0;
      $pack_qst_text = 0;
      $pack_total = $pack_price_tag;
      
      if ($setup4_pricevat == 1) {
        if($package_meta){
          $pack_gst = $package_meta['packageinfo_priceoutput_gst'];
          $pack_gst_text = $package_meta['packageinfo_priceoutput_gst_text'];
          $pack_qst = $package_meta['packageinfo_priceoutput_qst'];
          $pack_qst_text = $package_meta['packageinfo_priceoutput_qst_text'];
          $pack_total = $package_meta['packageinfo_priceoutput_text'];
        }
                                                  ...
                                                  <div class="pfinv-details">
                                                    <div class="pfinv-column -span-50">
                                                      <div class="pfinv-buyer">
                                                        <strong><?php esc_html_e('Depuis','pointfindert2d');?></strong><br>
                                                        <?php
                                                          if(!empty(trim($inv_user_company))){
                                                            echo trim($inv_user_company);
                                                          }
                                                        ?><br>
                                                        <?php echo $inv_user_title;?><br>
                                                        <?php echo $inv_user_address;?><br>
                                                        <?php
                                                          if(!empty(trim($inv_user_city))){
                                                            echo $inv_user_city;
                                                            if(!empty(trim($inv_user_country))){
                                                              echo ' ('.$inv_user_country.') ';
                                                              if(!empty(trim($inv_user_postalcode))){
                                                                echo ' '.$inv_user_postalcode;
                                                              }
                                                            }else{
                                                              if(!empty(trim($inv_user_postalcode))){
                                                                echo $inv_user_postalcode;
                                                    ...
                                                    <div class="pfinv-column -span-50 -last">
                                                      <div class="pfinv-seller">
                                                        <strong><?php esc_html_e('Pour','pointfindert2d');?></strong><br>
                                                      ...
                                                      <table>
                                                        <thead>
                                                          <tr>
                                                            ...
                                                            <th><?php esc_html_e('Package','pointfindert2d');?></th>
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                              ...
                                                              <td class="pfinv-td--price"><?php echo $pack_price_tag;?></td>
                                                            </tr>
                                                        </tbody>
                                                      </table>
                                                      <div class="pfinv-total">
                                                        <?php echo esc_html__('Subtotal','pointfindert2d').' : '.$pack_price_tag;?>
                                                      </div>
                                                      <?php
                                                        if($setup4_pricevat == 1){
                                                          ?>
                                                      <div class="pfinv-total">
                                                        <?php echo esc_html__('GST','pointfindert2d').' <span class="tax-no"></span> : '.$pack_gst_text;?>
                                                      </div>
                                                      <div class="pfinv-total">
                                                        <?php echo esc_html__("QST",'pointfindert2d').' <span class="tax-no"></span> : '.$pack_qst_text;?>
                                                      </div>
                                                      <div class="pfinv-total">
                                                        <?php echo esc_html__('Total','pointfindert2d').' : '.$pack_total;?>
                                                      </div>
                                                          <?php
                                                        }else{
                                                          ?>
                                                      <div class="pfinv-total">
                                                        <?php echo esc_html__('Total','pointfindert2d').' : '.$pack_price_tag;?>
                                                      </div>
                                                          <?php
                                                        }
                                                      ?>
                                                      <div class="pfinv-payment-method">
                                                        <?php echo esc_html__('','pointfindert2d').' '.$inv_paid_via;?>
                                                      </div>
                                                    </div>
                                                  </div>
