...
function pf_ajax_usersystem_child(){
...
**/
   ...
   if ( $pfrecheck == 1 && $pfrechecklg != 1) {$recaptcha_vars = '';}
    ...
    ?><script type='text/javascript'>(function($) {"use strict";$.pfAjaxUserSystemVars = {};$.pfAjaxUserSystemVars.username_err = '<?php echo esc_html__('Please write username','pointfindert2d');?>';$.pfAjaxUserSystemVars.username_err2 = '<?php echo esc_html__('Please enter at least 3 characters for Username.','pointfindert2d');?>';$.pfAjaxUserSystemVars.password_err = '<?php echo esc_html__('Please write password','pointfindert2d');?>';})(jQuery);</script>
      <div class="golden-forms wrapper mini">
        <div id="pflgcontainer-overlay" class="pftrwcontainer-overlay"></div>
        <form id="pf-ajax-login-form"><div class="pfmodalclose"><i class="pfadmicon-glyph-707"></i></div><div class="pfsearchformerrors"><ul></ul><a class="button pfsearch-err-button"><?php echo esc_html__('CLOSE','pointfindert2d');?></a></div><div class="form-title"><h2><?php echo esc_html__('Account Login','pointfindert2d');?></h2></div>
      <div class="form-enclose">
        <div class="form-section">
        <section>
          <label class="cxb"><?php echo esc_html__('Not a member yet?','pointfindert2d');?></label>
          <label class="cxb"><strong><a id="" class="glink ext" href="<?php echo get_permalink(6301); ?>"><?php echo esc_html__('Register Now','pointfindert2d');?></a></strong> <?php echo esc_html__('- Its  Free','pointfindert2d');?></label>
          <div class="tagline">
            <span><?php echo esc_html__('OR','pointfindert2d');?></span>
          </div>
        </section>
        <section><div class="pointfinder-login-scbuttons"><span class="pflgtext"><?php echo esc_html__('LOGIN WITH','pointfindert2d');?></span><span class="pflgbuttons"><?php echo $facebook_login_text;echo $twitter_login_text;echo $google_login_text;echo $vk_login_text;?></span></div><div class="tagline"></div></section>
        <section><label for="usernames" class="lbl-text"><?php echo esc_html__('Username:','pointfindert2d');?></label><label class="lbl-ui append-icon"><input type="text" name="username" class="input" placeholder="<?php echo esc_html__('Enter Username','pointfindert2d');?>" autofocus /><span><i class="pfadmicon-glyph-632"></i></span></label></section>
        <section><label for="pass" class="lbl-text"><?php echo esc_html__('Password:','pointfindert2d');?></label><label class="lbl-ui append-icon"><input type="password" name="password" class="input" placeholder="<?php echo esc_html__('Enter Password','pointfindert2d');?>" /><span><i class="pfadmicon-glyph-465"></i></span></label></section>
        <?php echo $recaptcha_vars;?>
        <section><span class="gtoggle"><label class="toggle-switch blue"><input type="checkbox" name="rem" id="toggle1_rememberme" /><label for="toggle1_rememberme" data-on="<?php echo esc_html__('YES','pointfindert2d');?>" data-off="<?php echo esc_html__('NO','pointfindert2d');?>"></label></label><label for="toggle1"><?php echo esc_html__('Remember me','pointfindert2d');?> <strong><a id="pf-lp-trigger-button-inner" class="glink ext"><?php echo esc_html__('Forgot Password?','pointfindert2d');?></a></strong></label></span></section>
        </div>
      </div>
      <div class="form-buttons">
        <section>
          <input type="hidden" name="redirectpage" value="<?php echo $redirectpage;?>"/>
          <button id="pf-ajax-login-button" class="button blue"><?php echo esc_html__('Login Now','pointfindert2d');?></button>
        </section>
      </div>
      </form>
      </div><?php
