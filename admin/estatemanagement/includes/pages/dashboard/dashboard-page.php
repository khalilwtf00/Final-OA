		...
		if($setup4_membersettings_dashboard != 0){
              ...
              if (!empty($membership_user_activeorder_ex)) {
                ...
                $pointfinder_order_bankcheck = '';
							...
							$pfmenu_output .= '<li class="pf-dash-userprof">
                <div class="pf-dash-packageinfo pf-dash-newpackage">';
              
							if (!empty($membership_user_package_id)) {
                if($membership_user_item_limit > 1){
                  $remaining_ads_text = esc_html__(' remaining ads','pointfindert2d');
                }else{
                  $remaining_ads_text = esc_html__(' remaining ad','pointfindert2d');
                }
								$pfmenu_output .= '<span class="pf-dash-packageinfo-tableex">'.$membership_user_item_limit.'</span><span class="pf-dash-packageinfo-table">'.$remaining_ads_text.'</span>';
							}
              
              $pfmenu_output .= '<button class="pf-dash-purchaselink">'.esc_html__('Purchase an ad package','pointfindert2d').'</button>';
                  $pfmenu_output .= "
									<script>
							...
							**/
								...
								if(isset($_GET)){
									if (isset($_GET['action'])) {
                                                          ...
                                                          PF_CreateInvoice(
                                                            ...
                                                            PF_CreateInvoice(
                                                              array(
                                                                ...
                                                                'processname' => esc_html__('Paypal Payment','pointfindert2d'),
                                                                'amount' => $packageinfo_n['packageinfo_priceoutput'],
                                                                'gst' => $packageinfo_n['packageinfo_priceoutput_gst'],
                                                                'qst' => $packageinfo_n['packageinfo_priceoutput_qst'],
                                                                'datetime' => strtotime("now"),
                                      ...
                                      update_post_meta( $result_id, 'pointfinder_order_expiredate', 3000000000 );
                                          ...
                                          'user_id' => $user_id,
                                          'item_post_id' => $item_post_id,
                                          'processname' => sprintf(esc_html__('Expire date extended by User (Free Listing): (Order Date: %s / Expire Date: %s)','pointfindert2d'), $app_date, $exp_date)
                                        )
																			);
