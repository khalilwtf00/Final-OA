  ...
  if(isset($_POST) && $_POST!='' && count($_POST)>0){
    if (esc_attr($_POST['action']) == 'pfget_updateuserprofile') {
      ...
      if($user_id != 0){
          ...
          if($current_user->user_login == $pf_username_old){
            if( $pf_username != $pf_username_old ) {
              ...
              if( $result_username === false ) {
                ...
                $sccval .= "<script type='text/javascript'>
                              (function($) {
                                'use strict';
                                $(function(){
                                  setTimeout(function() {
        ...
        update_user_meta($user_id, 'company', $vars['company']);
        ...
        if(isset($vars['postalcode'])){update_user_meta($user_id, 'user_postalcode', $vars['postalcode']);}
