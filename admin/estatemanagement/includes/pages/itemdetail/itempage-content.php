...
if (!function_exists('PFGetItemPageCol1')) {
    ...
    $setup42_itempagedetails_configuration['features'] = array('status'=>1);
    function moveElement(&$array, $a, $b) {
        $p1 = array_splice($array, $a, 1);
        $p2 = array_splice($array, 0, $b);
        $array = array_merge($p2,$p1,$array);
    }
    moveElement($setup42_itempagedetails_configuration, 15, 2);
		...
		foreach ($setup42_itempagedetails_configuration as $key => $value) {
            ...
            /** 
              ...
              //die(var_dump($featured_image));
              if(empty($featured_image[0])){
                $featured_image[0] = get_stylesheet_directory_uri().'/images/noimg_full.png';
              }
        ...
        case 'features':
          ...
          if ($features_list_permission == 1) {
            ...
            if ($setup3_pointposttype_pt6_check == 1) {
              ...
              if(count($featured_terms) > 0){
                ...
                if($i > 1){
                  $taboutside_w1 .= '<input class="ui-tab'.$i.'" type="radio" id="tgroup_f_tab'.$i.'" name="tgroup_f" '.$valtext.' />';
                  $taboutside_w2 .= '<label id="pfidp'.$key.'" class="ui-tab'.$i.' hidden-print" for="tgroup_f_tab'.$i.'"><span class="pfitp-title">'.$setup3_pointposttype_pt6.'</span></label>';
                }
              }
            }
          }
          break;
                ...
                global $wpdb;
                    ...
                    */
                          ...
                          if($user_posts > 0 && $setup42_itempagedetails_contact_moreitems == 1){
														$tabinside .= '<li class="pf-itempage-sidebarinfo-elurl pf-itempage-sidebarinfo-elitem"><a href="'.get_author_posts_url($user->ID).'"><i class="pfadmicon-glyph-510"></i> '.esc_html__('Other items','pointfindert2d').' ('.$user_posts.')</a></li>';
													}
                          */
                            ...
                            $tmp_user_url = $user->user_url;
                            if(strpos($tmp_user_url, "http://") === false
                              &&
                              strpos($tmp_user_url, "https://") === false
                            ){
                              $tmp_user_url = 'http://'.$tmp_user_url;
                            }
                            $tabinside .= '<li class="pf-itempage-sidebarinfo-elurl pf-itempage-sidebarinfo-elitem" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><a href="'.$tmp_user_url.'" target="_blank" rel="nofollow"><i class="pfadmicon-glyph-592"></i> '.$tmp_user_url.'</a></li>';
                          }
													$tabinside .= '</ul>
												</div>';
												$tabinside .= '</div>';
								...
								**/
									if ($show_agentcon == 1) {
										...
										$item_agent = $item_agents;
                                          ...
                                          <div class="pf-itempage-sidebarinfo-photo"><a href="'.get_permalink($item_agent).'" >'.$user_photo.'</a></div>';
													if(count($user_socials) > 0){
														$tabinside .= '<ul class="pf-itempage-sidebarinfo-social'.$css_text.' pf-row clearfix">';
													...
													$tabinside .= ' </div>
                                        <div class="col-lg-9 col-md-6 col-sm-6 col-xs-12 pfcols">';
												}else{
													if ($setup42_itempagedetails_contact_form == 0) {
														$tabinside .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pfcols">';
													}else{
												...
												$tabinside .= '<div class="pf-itempage-sidebarinfo-userdetails pfpos2'.$ex_text.'">
													<ul class="pf-authordetail-page">';
                          $tabinside .= '<li class="pf-itempage-sidebarinfo-elname"><a href="'.get_permalink($item_agent).'">'.$contact_user_title.'</a></li>';
                          $agent_description = get_post_field('post_content', $item_agent);
                          $processed_agent_description = apply_filters('the_content', $agent_description);
													if(!empty($agent_description)){
                            $tabinside .= '<li class="pf-itempage-sidebarinfo-eldesc pf-itempage-sidebarinfo-elitem">'.$processed_agent_description.'</li>';
                          }
			...
			$excludeobj_arr = array('twitter','description','video','events','streetview','gallery','features','customtab1','customtab2','customtab3','customtab4','customtab5','customtab6');
			$itemvalimg = '';
			...
			if ($value['status'] == 1 && ($key != 'events' && $key != 'details' && $key != 'ohours' && $key != 'twitter' && $key != 'description')) {
				$i++;
			}
